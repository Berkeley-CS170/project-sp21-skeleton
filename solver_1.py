...
def solve(G):
    ...
    while k_max > 0:
        shortest_path = nx.dijkstra_path(new_G, 0, num_nodes - 1)
        edge_weights = [new_G[shortest_path[i]][shortest_path[i+1]]["weight"] + 5 * new_G.degree[shortest_path[i]] + 5 * new_G.degree[shortest_path[i+1]] for i in range(len(shortest_path) - 1)]
